...
def getMatrix(dirname):
    ...
    m = len(pssmList)
    reMatrix = np.zeros((m,400))
    for i in range(m):
        matrix, labels = readToMatrix(dirname + '/' + pssmList[i], 'pssmAndLabels')
        matrix = autoNorm(matrix, 'pssm')
        reMatrix[i, :] = getRPT(matrix, labels)
...
def main():
    ...
    np.savetxt("NEW-10fold-PVP-final-PRT-probas_y.csv",loo_probas_y,delimiter=",")
    np.savetxt("NEW-10fold-PVP-final-PRT-predict_y.csv", loo_predict_y, delimiter=",")
    ...
    #np.savetxt("NEW-PVP-final-PRT-IND-probas_y.csv", probas_y, delimiter=",")
    #np.savetxt("NEW-PVP-final-PRT-IND-predict_y.csv", predict_y, delimiter=",")
